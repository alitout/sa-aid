...
import { ORG_UPDATEUSERBYID } from '../../../../externalApi/ExternalUrls';
import { globalVariables } from '../../../../Global/globalVariables';
import axios from 'axios';

const EditUser = ({ show, handleClose, userData, onSave, auth }) => {
    ...
    const [userPhoneNumber, setUserPhoneNumber] = useState(userData.UserPhoneNumber);
    const [isPhoneEmpty, setIsPhoneEmpty] = useState(false);
    const [isPhoneValid, setIsPhoneValid] = useState(true);
    const [userDOB, setUserDOB] = useState(new Date(userData.UserDOB).toISOString().split('T')[0]);
    const [isDOBEmpty, setIsDOBEmpty] = useState(false);
    const [userNationality, setUserNationality] = useState(userData.UserNationality);
    ...
    const [userRole, setUserRole] = useState(userData.UserRole);
    const [isRoleEmpty, setIsRoleEmpty] = useState(false);
    const [isUserHeadOfDistribution, setIsUserHeadOfDistribution] = useState(userData.isHeadOfDistribution);
    const [editUserSuccess, setEditUserSuccess] = useState();
    const [editUserFail, setEditUserFail] = useState();
    ...
    const editUser = async (e) => {
        ...
        if (!isFNameEmpty && !isLNameEmpty && isEmailValid && !isPhoneEmpty && isPhoneValid && !isDOBEmpty && !isNationalityEmpty && !isGenderEmpty && !isAddressEmpty && !isRoleEmpty) {
            ...
            try {
                const response = await axios.patch(`${ORG_UPDATEUSERBYID}${UserID}`, editUserRequest, {
                    headers: {
                        'Authorization': auth,
                    }
                });
                setEditUserFail(null);
                setEditUserSuccess(response.data.msg);
                setTimeout(() => {
                    onSave(editUserRequest);
                }, 3000);
            } catch (error) {
                console.error(error);
                setEditUserFail(error.response.msg);
            }
        };
    }
                        ...
                        <div className="d-flex flex-wrap justify-content-start row-gap-1 column-gap-3">
                            ...
                            <div className='input input-code form-check d-flex mb-1_25 gap-5 col-12 col-sm-4 flex-fill p-0'>
                                <input
                                    ...
                                    checked={isUserHeadOfDistribution}
                                />
